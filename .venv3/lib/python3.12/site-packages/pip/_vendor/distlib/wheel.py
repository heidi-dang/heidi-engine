        ...
        for arch in arches:
            ...
            if abi != "none" and sys.platform.startswith("linux"):
                ...
                if len(parts) == 2:
                    if parts >= (2, 5):
                        result.append(
                            ("".join((IMP_PREFIX, versions[0])), abi, "manylinux1_%s" % arch)
                        )
                    if parts >= (2, 12):
                        result.append(
                            ("".join((IMP_PREFIX, versions[0])), abi, "manylinux2010_%s" % arch)
                        )
                    if parts >= (2, 17):
                        result.append(
                            ("".join((IMP_PREFIX, versions[0])), abi, "manylinux2014_%s" % arch)
                        )
                    result.append(
                        (
