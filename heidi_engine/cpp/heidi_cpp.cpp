...
deduplicate_strings(const std::vector<std::string> &inputs) {
...
std::vector<bool> parallel_validate(const std::vector<std::string> &snippets,
                                    int threads) {
...
std::string compress_data(const std::string &data) {
  ...
  if (compress(buffer.data(), &destLen, (const Bytef *)data.data(),
               data.size()) != Z_OK) {
...
void transpose_inplace(py::array_t<float> matrix, size_t rows, size_t cols) {
  ...
  if (rows != cols) {
    throw std::runtime_error("In-place transpose currently only supports "
                             "square matrices in this version");
  }
...
std::vector<std::string>
dedup_with_custom_hash(const std::vector<std::string> &inputs) {
...
std::vector<py::bytes> compress_logs(const std::vector<std::string> &logs) {
  ...
  for (const auto &log : logs) {
    ...
    if (compress(buf.data(), &dest_len,
                 reinterpret_cast<const Bytef *>(log.data()),
                 source_len) == Z_OK) {
      compressed.emplace_back(reinterpret_cast<const char *>(buf.data()),
                              dest_len);
    } else {
...
void run_with_limits(const std::function<void()> &func, int max_threads,
  ...
  // Note: max_threads enforcement would typically be done via OpenMP or pool
  // control. We'll execute the function as a wrapper.
  func();
}

// 11. Kernel-Aware Resource Governor
void run_with_kernel_bounds(const std::function<void()> &func, int max_jobs,
                            double cpu_limit, double mem_limit) {
  ...
  heidi::GovernorResult result =
      governor.decide(10.0, 10.0, 0, 0); // Mocked current usage
...
PYBIND11_MODULE(heidi_cpp, m) {
  ...
  m.def("deduplicate_strings", &deduplicate_strings,
        "O(1) string deduplication using unordered_set");
  m.def("sort_batch_inplace", &sort_batch_inplace,
        "In-place batch sorting for NumPy float arrays");
  m.def("parallel_validate", &parallel_validate,
        "Multi-threaded string validation", py::arg("snippets"),
        py::arg("threads") = 4);
  m.def(
      "compress_data",
  ...
  m.def("get_free_gpu_memory", &get_free_gpu_memory,
        "Get free GPU memory bytes (requires CUDA)");

  // Phase 3 bindings
  m.def("transpose_inplace", &transpose_inplace,
        "In-place matrix transpose (square only)");
  m.def("dedup_with_custom_hash", &dedup_with_custom_hash,
        "Deduplication with custom cache-aware hash");
  m.def("compress_logs", &compress_logs,
        "Batch compress a list of log strings");
  m.def("run_with_limits", &run_with_limits,
        "Run a Python function under resource limits", py::arg("func"),
        py::arg("max_threads") = 0, py::arg("max_memory_mb") = 0);
  m.def("run_with_kernel_bounds", &run_with_kernel_bounds,
        "Run a Python function under kernel-managed resource bounds",
        py::arg("func"), py::arg("max_jobs") = 10, py::arg("cpu_limit") = 85.0,
        py::arg("mem_limit") = 90.0);
