...
class CopilotProvider(LLMProvider):
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        # New gh copilot CLI uses -p for prompt and --model for selection.
        # --allow-all (or --yolo) ensures non-interactive tool execution if needed.
        cmd = ["gh", "copilot", "-p", prompt, "--model", model, "--allow-all"]
        ...
        try:
            ...
            # The output may contain usage stats at the end, but usually we just want the response.
            # If the output is empty but there's stderr, check stderr.
            if not output and result.stderr:
                output = result.stderr.strip()
                
            self.log_metrics(f"gh-copilot:{model}", latency_ms, None, None, "ok")
            return output
        except subprocess.CalledProcessError as e:
            latency_ms = int((time.time() - start_time) * 1000)
            self.log_metrics(f"gh-copilot:{model}", latency_ms, None, None, "error", error_kind="subprocess_error")
            raise ProviderError(f"Copilot generation failed (exit {e.returncode}): {e.stderr}")
