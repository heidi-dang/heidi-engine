...
REFRESH_RATE = int(os.environ.get("DASHBOARD_REFRESH_RATE", "5"))
...
AUTOTRAIN_DIR = os.environ.get("AUTOTRAIN_DIR", os.path.join(os.getcwd(), "autotrain_repos"))
...
def create_header(state: Dict[str, Any]) -> Panel:
    ...
    if last_update_str:
        try:
            ...
            # Get current time in same TZ as ISO string (UTC)
            now = datetime.now(timezone.utc)
            delta = (now - lu).total_seconds()
            
            if delta < 120:
                health_text.append("ACTIVE", style="bold green")
            elif delta < 600:
                health_text.append(f"IDLE ({int(delta/60)}m)", style="bold yellow")
            else:
                health_text.append(f"STALLED ({int(delta/60)}m)", style="bold red")
        except Exception as e:
            health_text.append(f"ERR", style="bold yellow")
    else:
        health_text.append("WAITING...", style="bold blue")
    content.append(health_text)
    ...
    # Progress Bar
    target = state.get("target_repos", 0)
    current = state.get("counters", {}).get("teacher_generated", 0)
    
    from rich.console import Group
    if target > 0:
        pct = min(1.0, float(current) / float(target)) if target else 0.0
        prog_text = Text(f"\nProgress: {current}/{target} ({pct*100:.1f}%)", style="dim")
        # Render a percentage-based progress bar from 0 -> 100
        bar_width =  min(60, console.size.width - 20) if console.size.width else 40
        bar = ProgressBar(total=100, completed=int(pct * 100), width=bar_width)
        panel_content = Group(content, prog_text, bar)
    else:
        panel_content = content

    return Panel(
        panel_content, title="[bold]Heidi AutoTrain Dashboard[/bold]", border_style=PANEL_STYLES["header"]
    )
...
def create_counters_panel(state: Dict[str, Any]) -> Panel:
    ...
    # Repo cloning progress (show per-bucket target if available)
    repos_cloned = counters.get("repos_cloned", 0)
    target_repos = state.get("target_repos", None)
    per_bucket_target = None
    if target_repos:
        try:
            per_bucket_target = int(int(target_repos) / 3)
        except Exception:
            per_bucket_target = None

    if per_bucket_target:
        # show per-bucket style if we can
        table.add_row("Repos Cloned (per-bucket)", f"{int(repos_cloned/3)} / {per_bucket_target}")
    else:
        if repos_cloned > 0:
            table.add_row("Repos Cloned", str(repos_cloned))

    # Countdown to generation start (if set in state)
    gen_ts = state.get("generation_start_ts")
    if gen_ts:
        try:
            from datetime import datetime, timezone

            dt = datetime.fromisoformat(gen_ts.replace("Z", "+00:00"))
            now = datetime.now(timezone.utc)
            secs = int((dt - now).total_seconds())
            if secs < 0:
                secs = 0
            table.add_row("Start In", f"{secs}s")
        except Exception:
            pass
...
def create_daemon_log_panel(lines: int = 12) -> Panel:
    """
    Tail the daemon_output.log and show the last `lines` lines.
    """
    log_path = Path(os.getcwd()) / "daemon_output.log"
    text = Text()
    if log_path.exists():
        try:
            ...
            text.append(f"Failed to read daemon log: {e}\n", style="red")
    else:
        text.append("daemon_output.log not found\n", style="yellow")

    return Panel(text, title="[bold]Daemon Log[/bold]", border_style=PANEL_STYLES["error"])


def create_processes_panel() -> Panel:
    """
    Show relevant processes (heidid, run_enhanced, git, gh, python) with basic stats.
    Falls back to raw `ps` output if `psutil` not available.
    """
    table = Table(box=box.MINIMAL, show_header=True, header_style="bold cyan")
    table.add_column("PID", style="magenta", width=6)
    table.add_column("STAT", style="yellow", width=6)
    table.add_column("%CPU", style="green", width=6, justify="right")
    table.add_column("%MEM", style="green", width=6, justify="right")
    table.add_column("Cmd", style="white")

    try:
        import psutil

        keywords = ("heidid", "run_enhanced", "git", "gh ", "python3")
        for p in psutil.process_iter(attrs=["pid", "name", "cmdline", "status", "cpu_percent", "memory_percent"]):
            cmd = " ".join(p.info.get("cmdline") or [])
            if any(k in cmd or k in (p.info.get("name") or "") for k in keywords):
                pid = str(p.info.get("pid"))
                stat = p.info.get("status")[:6]
                cpu = f"{p.info.get('cpu_percent'):.1f}"
                mem = f"{p.info.get('memory_percent'):.1f}"
                table.add_row(pid, stat, cpu, mem, cmd[:120])
    except Exception:
        # Fallback to ps
        try:
            out = subprocess.check_output(["ps", "-eo", "pid,stat,%cpu,%mem,cmd"], text=True)
            for line in out.splitlines()[1:]:
                if any(k in line for k in ("heidid", "run_enhanced", "git clone", "gh auth", "python3 heidi_engine")):
                    parts = line.split(None, 4)
                    if len(parts) >= 5:
                        pid, stat, cpu, mem, cmd = parts
                        table.add_row(pid, stat, cpu, mem, cmd[:120])
        except Exception as e:
            table.add_row("-", "-", "-", "-", f"ps failed: {e}")

    return Panel(table, title="[bold]Processes[/bold]", border_style=PANEL_STYLES["usage"])
...
def create_overview_layout(state: Dict[str, Any]) -> Layout:
    ...
    # Top row: counters, usage, processes
    top = Layout(name="top")
    top.split_row(
    ...
    # Bottom row: trainer, events, daemon log
    bottom = Layout(name="bottom")
    bottom.split_row(
        Layout(name="trainer", ratio=1),
        Layout(name="events", ratio=1),
        Layout(name="daemon", ratio=1),
    )
    ...
    layout["daemon"].update(create_daemon_log_panel())
    ...
    while True:
        try:
            if not sys.stdin.isatty():
                # Non-interactive mode, auto-select latest
                return runs[0]
