...
def create_header(state: Dict[str, Any]) -> Panel:
    ...
    return Panel(content, title="[bold]Heidi AutoTrain Dashboard[/bold]", border_style=PANEL_STYLES["header"])
...
def create_counters_panel(state: Dict[str, Any]) -> Panel:
    ...
    return Panel(table, title="[bold]Pipeline Progress[/bold]", border_style=PANEL_STYLES["counters"])
...
def create_events_panel() -> Panel:
    ...
    return Panel(table, title="[bold]Recent Events[/bold]", border_style=PANEL_STYLES["events"], height=15)
