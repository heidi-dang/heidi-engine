...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    # Path(run_id).name returns only the last component, stripping absolute paths or ..
    clean_id = Path(run_id).name
    if not clean_id or clean_id in (".", ".."):
        # Fallback to current run_id if sanitization results in an empty or special marker
        clean_id = get_run_id()
