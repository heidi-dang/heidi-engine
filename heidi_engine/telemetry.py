...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        - Sanitizes run_id using Path(run_id).name to prevent traversal
    ...
    # Path(run_id).name ensures we only use the base filename/dirname.
    # This specifically addresses the behavior where Path(a) / Path(b)
    # returns b if b is an absolute path.
