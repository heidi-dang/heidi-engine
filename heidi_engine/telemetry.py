...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Uses .name to ensure ID is a single component
    ...
    # Path(run_id).name extracts only the last component, preventing
    # absolute paths or ".." components from escaping the runs/ directory.
...
def get_state(run_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # This secondary check ensures that if another thread updated the cache
    # while we were checking state_file.exists(), we still use the cached version.
    cached = _state_cache.get(resolved_run_id)
    if cached:
        return cached
