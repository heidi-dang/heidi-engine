...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        - Sanitizes run_id to prevent path traversal (absolute paths, ..)
        - Falls back to get_run_id() for invalid inputs

    TUNABLE:
        - Modify directory structure by changing path construction
    """
    if run_id:
        # SECURITY: Use .name to strip any directory components/absolute paths
        run_id = Path(run_id).name

    # Fallback if run_id is empty, None, or was sanitized to empty (e.g. ".", "..")
    if not run_id:
        run_id = get_run_id()
