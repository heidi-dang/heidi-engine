...
_pricing_cache_key: Optional[str] = None
...
def load_pricing_config() -> Dict[str, Dict[str, float]]:
    ...
    BOLT OPTIMIZATION:
        Uses a thread-safe cache with a 2.0s TTL to avoid redundant disk I/O
        and JSON parsing on the performance-critical cost estimation path.
    ...
    TUNABLE:
        ...
        - Prices are per 1M tokens
    """
    global _pricing_cache, _pricing_cache_key, _pricing_last_check

    # Use Path(PRICING_CONFIG_PATH) or run_id-specific path as cache key
    cache_key = PRICING_CONFIG_PATH or get_run_id()

    with _pricing_lock:
        now = time.monotonic()
        if _pricing_cache and _pricing_cache_key == cache_key and (now - _pricing_last_check) < 2.0:
