...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Uses Path(run_id).name to only take the last component.
        - Fallbacks to a unique ID if name is empty or dangerous.
    ...
    # SECURITY: Prevent path traversal by only taking the filename component.
    # Also handles cases like '.', '..', or empty strings.
    safe_run_id = Path(run_id).name
    if not safe_run_id or safe_run_id in (".", ".."):
        # Fallback to current run_id if provided ID is dangerous
        # or generate a safe one if it's completely empty.
        safe_run_id = str(uuid.uuid4())[:8]
