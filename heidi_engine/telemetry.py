...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Ensures run_id is just a filename component
    ...
    # SECURITY: Sanitize run_id to prevent directory traversal.
    # Path(run_id).name extracts only the last component.
    # We also handle empty, '.' and '..' which .name might return as '' or '..'.
    clean_id = Path(run_id).name
    if not clean_id or clean_id in (".", ".."):
        # Fallback to current run_id if valid, or a safe default
        clean_id = "default" if not run_id else "invalid_run_id"

    return Path(AUTOTRAIN_DIR) / "runs" / clean_id
