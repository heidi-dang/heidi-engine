...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Ensures run_id stays within AUTOTRAIN_DIR/runs
    ...
    # Path(run_id).name returns only the last component of the path.
    # If it's empty, '.', or '..', we fall back to get_run_id().
    safe_run_id = Path(run_id).name
    if not safe_run_id or safe_run_id in (".", ".."):
        # Fallback to current run_id if provided one is unsafe
        safe_run_id = get_run_id()
        # Double check the fallback isn't also unsafe (e.g. if RUN_ID env var is "..")
        if not Path(safe_run_id).name or Path(safe_run_id).name in (".", ".."):
            # Last resort: generate a fresh safe ID
            import uuid
            safe_run_id = f"run_{uuid.uuid4().hex[:8]}"
        else:
            safe_run_id = Path(safe_run_id).name
