...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        - Sanitizes run_id to prevent directory traversal.
        - Uses Path(run_id).name to ensure it's a single directory component.
    ...
    sanitized_id = Path(run_id).name
    if not sanitized_id or sanitized_id == "..":
        # Fallback to current run_id if sanitization results in empty or dangerous path
        sanitized_id = get_run_id()
