...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Uses Path(run_id).name to ensure it's a relative filename
    ...
    # Path(run_id).name extracts only the filename/last part of the path,
    # preventing absolute paths or '..' from overriding the base directory.
    safe_run_id = Path(run_id).name

    # Handle cases where .name is empty or special markers
    if not safe_run_id or safe_run_id in (".", ".."):
        # If the provided run_id is unsafe, fallback to a safe generated ID
        safe_run_id = get_run_id() if run_id != get_run_id() else "default_run"
