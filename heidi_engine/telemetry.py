...
# BOLT OPTIMIZATION: Pre-compile secret patterns for faster redaction.
...
def redact_secrets(text: str) -> str:
    ...
    # Redact secrets
    # BOLT OPTIMIZATION: Use pre-compiled patterns for ~10% speedup on hot-path.
...
# BOLT OPTIMIZATION: Cache for pricing configuration to avoid redundant disk I/O.
...
def load_pricing_config() -> Dict[str, Dict[str, float]]:
    ...
    BOLT OPTIMIZATION:
        Thread-safe caching with mtime-based validation to avoid redundant
        disk I/O and JSON parsing on every call.
    ...
    with _pricing_lock:
        try:
            if pricing_file.exists():
                ...
                cache_key = str(pricing_file)

                if cache_key in _pricing_cache and _pricing_mtimes.get(cache_key) == mtime:
                ...
                _pricing_mtimes[cache_key] = mtime
...
def save_state(state: Dict[str, Any], run_id: Optional[str] = None) -> None:
    ...
    # BOLT OPTIMIZATION: Write-through update to StateCache for zero-latency subsequent reads.
    _state_cache.set(resolved_run_id, state)
