...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Only uses the filename component of the provided run_id.
    ...
    # We also handle edge cases like '.', '..', and empty strings.
    clean_run_id = Path(run_id).name
    if not clean_run_id or clean_run_id in (".", ".."):
        # Fallback to a safe unique ID if input is dangerous or empty
