...
def get_run_dir(run_id: Optional[str] = None) -> Path:
    ...
    SECURITY:
        ...
        - Uses Path(run_id).name to strip directory components
    ...
    # Path(base) / run_id can be exploited if run_id is an absolute path
    # or contains '..'. Path.name strips all directory components.
    safe_run_id = Path(run_id).name

    # Handle edge cases where .name might be empty or '.' or '..'
    if safe_run_id in (".", "..", ""):
        # Fall back to a default if run_id was just '.' or '..' or '/'
