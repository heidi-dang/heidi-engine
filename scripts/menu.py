...
def configure_parameters(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    config["TEACHER_MODEL"] = get_input("  Teacher model", config.get("TEACHER_MODEL", "gpt-4o-mini"))
    
    # Dataset configuration
    print("\n  --- Dataset Configuration ---")
    config["SAMPLES_PER_ROUND"] = get_input("  Samples per round", str(config.get("SAMPLES_PER_ROUND", 50)))
    ...
    # Options
    print("\n  --- Options ---")
    unit_tests = get_input("  Run unit tests (0/1)", str(config.get("RUN_UNIT_TESTS", "0")), ["0", "1"])
    config["RUN_UNIT_TESTS"] = unit_tests
    
    config["SEED"] = get_input("  Random seed", str(config.get("SEED", "42")))
    
    # Convert to proper types
    for key in ["SAMPLES_PER_ROUND", "ROUNDS", "SEQ_LEN", "LORA_R", "GRAD_ACCUM", "TRAIN_STEPS", "BATCH_SIZE", "SAVE_STEPS", "EVAL_STEPS"]:
        if key in config:
            try:
...
def main():
    ...
    interactive_only = not any([
        args.status, args.list, args.start, args.stop,
        args.pause, args.resume, args.dashboard, args.http
    ])
