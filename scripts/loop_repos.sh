...
REPOS_FILE=""
if [ -f "repos.txt" ]; then REPOS_FILE="repos.txt"; fi
GH_QUERY=""
...
while true; do
    ...
    if [[ "$repo" =~ ^https?:// ]] || [[ "$repo" =~ \.git$ ]]; then
        ...
        if [ -d "$repo" ]; then
            ...
            mkdir -p "$target_dir"
            # Use rsync if available to avoid copying the output directory into itself
            if command -v rsync >/dev/null 2>&1; then
                rsync -a --exclude="$(basename "$OUT_BASE")" "$repo/" "$target_dir/"
            else
                # Fallback to cp but try to be careful
                cp -r "$repo/." "$target_dir/" 2>/dev/null || true
            fi
        else
