...
def evaluate_format_compliance(output: str, task_type: str = "") -> Dict[str, Any]:
    ...
    result["has_error_indicators"] = any(
        ind in output.lower() for ind in error_indicators
    )
    
    # Format compliance: passes if has content and no errors
    result["format_compliant"] = (
        result["has_content"] and 
        not result["has_error_indicators"]
    )
...
def evaluate_sample(
    ...
    format_eval = evaluate_format_compliance(
        output, 
        sample.get("metadata", {}).get("task_type", "")
    )
...
def main():
    ...
    print(f"[INFO] Loading model and adapter...")
