...
Examples:
  ...
  python 03_unit_test_gate.py --input data/clean.jsonl --output data/tested.jsonl \\
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Input JSONL file (cleaned data)"
    )
    parser.add_argument(
        "--output", "-o",
        type=str,
        required=True,
        help="Output JSONL file (tested data)"
    )
    parser.add_argument(
        "--timeout",
        type=int,
...
def main():
    ...
    for i, sample in enumerate(samples):
        ...
        # Progress
        if (i + 1) % 10 == 0:
            print(f"  Tested {i + 1}/{len(samples)} samples "
                  f"(passed: {passed_count}, failed: {failed_count})", file=sys.stderr)
    ...
    # Summary
    print(f"[OK] Unit test gate complete!")
