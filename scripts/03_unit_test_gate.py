...
def test_python_code(code: str, temp_dir: str, execution_timeout: int = 5) -> Tuple[bool, str, str]:
    ...
    # SECURITY: Indent the user's code before wrapping it in the template
    # to ensure it's valid within the 'try' block and doesn't cause syntax errors.
...
"""
    ...
    # SECURITY: Filter environment variables to prevent leaking host secrets
    # to the generated code during execution.
    safe_env = {
        k: v
        for k, v in os.environ.items()
        if not any(
            secret_keyword in k.upper()
            for secret_keyword in ["KEY", "TOKEN", "SECRET", "PASS"]
        )
    }
