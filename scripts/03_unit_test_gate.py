...
DANGEROUS_PATTERNS = [
    ...
    # Dangerous built-ins and attributes
    r"\beval\s*\(",
    r"\bexec\s*\(",
    r"\b__import__\s*\(",
    r"\bgetattr\s*\(",
...
"""
    ...
    # SECURITY: Filter environment to prevent leaking secrets to untrusted code
    safe_env = {
        k: v for k, v in os.environ.items()
        if not any(s in k.upper() for s in ["KEY", "TOKEN", "SECRET", "PASS"])
    }
