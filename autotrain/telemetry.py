...
ALLOWED_STATUS_FIELDS: Set[str] = {
    "run_id", "status", "current_round", "current_stage",
    "stop_requested", "pause_requested", "counters", "usage",
    "gpu_summary", "last_event_ts", "health", "updated_at"
}
...
def load_pricing_config() -> Dict[str, Dict[str, float]]:
    ...
    pricing_file = Path(PRICING_CONFIG_PATH) if PRICING_CONFIG_PATH else get_run_dir() / "pricing.json"
...
def main():
    ...
    if args.command == "init":
        ...
        emit_event(args.type, args.message, stage=args.stage, round_num=args.round, run_id=args.run_id)
        flush_events()
        print("Event emitted")
